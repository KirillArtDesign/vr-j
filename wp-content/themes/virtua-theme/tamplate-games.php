<?php
/*
Template Name: Шаблон игр
*/

if($_GET['genre']!="" and $_GET['genre']!="all") {
	
	wp_redirect( get_term_link( htmlspecialchars(strip_tags($_GET['genre'])), 'games-genre' ), 301 );
	exit;
	
} elseif($_GET['type']!="" and $_GET['type']!="all") {
	
	wp_redirect( get_term_link( htmlspecialchars(strip_tags($_GET['type'])), 'games-ustroistva' ), 301 );
	exit;
