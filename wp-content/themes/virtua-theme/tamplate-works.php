...
<!--section-->
				<div class="col-md-9 col-xs-12">
					...
					<div class="block">
							...
							$args = array(
									   'post_type' => array('vacancies','resume'), /* on_resume */
									   'publish' => true,
									   'posts_per_page' => 20,
						...
						//		$args['meta_compare']='LIKE';
							...
							<?php endif; ?>
								...
								<div class="filter filter_vacancy">
									...
									<div class="filter__form">
										<div class="filter__vacancy-cell filter__vacancy-cell_text filter__vacancy-cell_prof">
											<input type="text" name="on-name" class="filter__input filter__input_vacancy" value="<? echo $_GET['on-name']; ?>" placeholder="Резюме или вакансия">
										</div>
								...
								<div class="sort">
									...
									<a href="<?=$url_filter?>" class="sort__active sort__up">Дате добавления</a>
									<a href="<?php echo get_permalink('408');?>">Вакансии</a>
								...
								<?php
									if ( have_posts() ) : ?>
										...
										while ( have_posts() ) : the_post();
												...
												//Картинка
												if ($post->post_type=='vacancies') { $img_adds = get_the_post_thumbnail_url(get_post_meta($post->ID,'on_vacancy_id_company',true), 'full' ); }
												else { $img_adds = get_the_post_thumbnail_url($post->ID, 'full' ); }
												...
												if ($post->post_type=='resume') {
														$ot =  number_format(get_post_meta($post->ID,'on_resume_jelanie_zp_ot',true), 0, '.', ' ');
														$do =  number_format(get_post_meta($post->ID,'on_resume_jelanie_zp_do',true), 0, '.', ' ');
