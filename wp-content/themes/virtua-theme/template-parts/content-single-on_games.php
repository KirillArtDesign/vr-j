<?php
/**
 * The template used for displaying page content in page.php

 */

?>
							<div class="article">
								<? the_title( '<h1 class="block__title"><span>', '</span></h1>' ); ?>
								<div class="game">
									<div class="row">
										<div class="game__gallery col-xs-12">
											<div class="game__thumb">
													<?php the_post_thumbnail('original'); ?>	
											</div>
											<div class="game__thumb game__thumb_video">
												<? echo do_shortcode('[video src="'.get_post_meta ($post->ID,'on_game_video',true).'"]'); ?>
											</div>
											<div class="game__thumb game__carousel">
												<?php
												$images = twp_get_post_images($post->ID);
												foreach ($images as $im) {
												$thumb_url = wp_get_attachment_image_src($im->id, 'exlusive-post-thumb', true);
												$thumb_url_orig = wp_get_attachment_image_src($im->id, 'large', true);
												?>
													<div class="game__carousel-item">
														<?/*  print_r($thumb_url); */ ?>
														<a href="<?php echo $thumb_url_orig[0]; ?>" class="modal" rel="group"><img src="<?php echo $thumb_url[0]; ?>"></a>
													</div>
												<?php
												}
												?>
											</div>
										</div>
										<div class="game__content col-xs-12">
											<div class="rating rating__game"><?php echo get_post_meta ($post->ID,'on_game_rating',true); ?></div>
											<div class="game__content-title">
												 <br><br>
											</div>
											<div class="game__block-gray">
												Рейтинг: Steam: <?php echo get_post_meta ($post->ID,'on_game_otsenka_stream',true); ?> | PlayStation Store: <?php echo get_post_meta ($post->ID,'on_game_otsenka_ps',true); ?>
											</div>
											<div class="game__block-gray">
												<?php if(function_exists('the_ratings')) { the_ratings(); } ?>
											</div>
											<div class="game__info">
												<div><strong>Жанр:</strong> <?php echo get_the_term_list( $post->ID, 'taxonomy_games_genre', ' ', ',', '' ); ?></div>
												<div><strong>Разработчик: </strong> <?php echo get_the_term_list( $post->ID, 'taxonomy_games_razrab', ' ', ',', '' ); ?></div>
												<div><strong>Издатель: </strong> <?php echo get_the_term_list( $post->ID, 'taxonomy_games_izdatel', ' ', ',', '' ); ?></div>
												<div><strong>Доступные платформы:</strong> <?php echo get_the_term_list( $post->ID, 'taxonomy_games_platform', ' ', ',', '' ); ?></div>
												<div><strong>Поддерживаемые устройства:</strong> <?php echo get_the_term_list( $post->ID, 'taxonomy_games_ustroistva', ' ', ',', '' ); ?></div>
												<div><strong>Дата релиза:</strong> <?php echo get_post_meta ($post->ID,'on_game_data_realise',true); ?></div> 
												<div><strong>Системные требования: </strong> <?php echo get_post_meta ($post->ID,'on_game_sis',true); ?></div>
												<div><strong>Наличие русской локализации: </strong> <?php echo get_post_meta ($post->ID,'on_game_rus_locate',true); ?></div>
											</div>
											<div class="game__desc">
												<strong>Описание: </strong> <?php the_content(); ?>
											</div>
										</div>
									</div>
								</div>
							</div>